...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow {
		...
		private int movingIndicator;
		public Color nodeIndicationColor = Color.blue;
                ...
                case NoodlePath.Curvy:
                    ...
                    for (int i = 0; i < length - 1; i++) {
						...
						for (int j = 1; j <= division; ++j) {
							...
							if (!indicated && movingIndicator < i)
							{
								if (movingIndicator < length)
									movingIndicator++;
								else
									movingIndicator = 0;
								Handles.color = nodeIndicationColor;
								indicated = true;
							}
							else
								movingIndicator = 0;
        ...
        private void DrawNodes() {
            ...
            for (int n = 0; n < graph.nodes.Count; n++) {
				...
				if (graph is DialogGraph)
				{
					...
					{
						RenderDialogNodeGUI(nodeEditor, node);
					}
					else if (node is DialogEventNode)
					{
						RenderEventNodeGUI(nodeEditor, node);
					}
				}
				...
				if (EditorGUI.EndChangeCheck()) {
					...
					if (graph is DialogGraph)
					{
						SerializedObject so = new SerializedObject((graph as DialogGraph).dialogObject);
						EditorUtility.SetDirty((graph as DialogGraph).dialogObject);
		...
		private DialogNode dn;
		private DialogEventNode dENode;
