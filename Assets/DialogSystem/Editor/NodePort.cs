...
namespace XNode {
    ...
    public class NodePort {
		...
		[SerializeField] private int _fieldID;
        ...
        public NodePort(string fieldName, Type type, IO direction, Node.ConnectionType connectionType, Node.TypeConstraint typeConstraint, Node node, int id) {
        ...
        public void Connect(NodePort port) {
			...
			if (port.IsOutput && port.IsConnected)
				port.DisconnectAll();
			if (this.IsOutput && this.IsConnected)
				DisconnectAll();
			if (port == this)
				return;
#if UNITY_EDITOR
			...
			if (IsOutput)
				Debug.Log("Connecting output of " + fieldID + " to input of " + (port.node));
        }
