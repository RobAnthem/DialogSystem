...
public class LipsyncController : MonoBehaviour
{
	...
	private List<float> textFrequencies;
	/// <summary>
	...
	void UpdateBlendShapes()
	{
		...
		if (mode == ModeType.Speech)
			...
			movement = CalcSpectrum(frqLow, frqHigh);
		if (ignoreDistance)
		{
	...
	void UpdateJawBone()
	{
		...
		float movementValue = Mathf.Clamp(mAV, 0, 100);
		lastMoveValue = Mathf.MoveTowards(lastMoveValue, movementValue, mouthMoveSpeed * Time.deltaTime);
		mAV = lastMoveValue;
		Vector3 pos = mouth.transform.localPosition;
		Quaternion rot = mouth.transform.localRotation;
	...
	public void GetTextFreq(string text, float d)
	{
		textFrequencies = new List<float>();
		for (int i = 0; i < text.Length; i++)
		{
			textFrequencies.Add(TextFrequencies.Instance.GetFreq(text[i]));
		}
		currentTime = 0;
		duration = d;
	}
	public float SampleText()
	{
		if (currentTime >= duration)
			return 0;
		int index = (int)(textFrequencies.Count * Mathf.InverseLerp(0, duration, currentTime));
		index = Mathf.Clamp(index, 0, textFrequencies.Count - 1);
		return textFrequencies[index];
	}
